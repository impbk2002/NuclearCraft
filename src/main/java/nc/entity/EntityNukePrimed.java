...
public class EntityNukePrimed extends Entity {
	...
	public void onUpdate() {
        ...
        if (this.fuse-- <= 0) {
            ...
            if (!this.worldObj.isRemote) {
                this.explode(worldObj, this.posX, this.posY, this.posZ, 0.6F*Config.explosionRadius, BombType.BOMB_STANDARD);
            }
        }
        else {
	...
	private void explode(World world, double x, double y, double z, float radius, BombType type) {
        NCExplosion.createExplosion(new EntityBomb(world).setType(type), world, (double)this.posX, (double)this.posY, (double)this.posZ, radius, Config.explosionRadius, true);
    }
