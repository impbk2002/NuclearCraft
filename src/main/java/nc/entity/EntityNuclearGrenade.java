...
public class EntityNuclearGrenade extends EntityThrowable {
	...
	protected void onImpact(MovingObjectPosition mop) {
		if(mop.typeOfHit == MovingObjectType.BLOCK) {
			...
			if (!this.worldObj.isRemote) {
				this.explode(worldObj, this.posX, this.posY, this.posZ, 0.3F*Config.explosionRadius, BombType.BOMB_STANDARD);
				this.setDead();
			}
		}
	}
	...
	private void explode(World world, double x, double y, double z, float radius, BombType type) {
		NCExplosion.createExplosion(new EntityBomb(world).setType(type), world, (double)this.posX, (double)this.posY, (double)this.posZ, radius, 0.5F*Config.explosionRadius, true);
	}
}